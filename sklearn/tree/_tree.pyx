...
cdef struct ReducedErrorPruningRecord:
    SIZE_t node_idx
    SIZE_t parent

cdef _reduced_error_prune(unsigned char[:] leaves_in_subtree, # OUT
                            Tree orig_tree, SIZE_t offset):
    """Perform reduced error pruning.
    MJB TO DO- Update This Comment
    This function takes an already grown tree, `orig_tree` and outputs a
    boolean mask `leaves_in_subtree` which are the leaves in the pruned tree.
    During the pruning process, the controller is passed the effective alpha and
    the subtree impurities. Furthermore, the controller signals when to stop
    ...
    cdef:
        ...
        stack[ReducedErrorPruningRecord] rep_stack
        ReducedErrorPruningRecord stack_record
        SIZE_t node_idx
        stack[SIZE_t] node_indices_stack
        ...
        # nodes in subtree
        unsigned char[:] in_subtree = np.ones(shape=n_nodes, dtype=np.uint8)
        SIZE_t pruned_branch_node_idx = -1
        unsigned char[:] candidate_nodes = np.zeros(shape=n_nodes,
                                                    dtype=np.uint8)
    pruned = False
    # find parent node ids and leaves
    with nogil:
        # Push the root node
        rep_stack.push({"node_idx": 0, "parent": _TREE_UNDEFINED})

        while not rep_stack.empty():
            stack_record = rep_stack.top()
            rep_stack.pop()
            ...
            if child_l[node_idx] == _TREE_LEAF:
                ...
                rep_stack.push({"node_idx": child_l[node_idx], "parent": node_idx})
                rep_stack.push({"node_idx": child_r[node_idx], "parent": node_idx})

        for i in range(leaves_in_subtree.shape[0]):
            candidate_nodes[i] = not leaves_in_subtree[i]

        for i in range(candidate_nodes.shape[0]-offset, 0, -1):
            if candidate_nodes[i]:
                pruned_branch_node_idx = i
                pruned = True
                break

        if pruned_branch_node_idx <= 0:
            with gil:
                return
...
def _build_pruned_tree_rep(
    Tree tree, # OUT
    Tree orig_tree,
    object validation_set_X,
    const DOUBLE_t[:, ::1] validation_set_y,
    SIZE_t offset):
    """Build a pruned tree from the original tree using reduced error
    pruning.
    ...
    cdef:
        ...
        unsigned char[:] leaves_in_subtree = np.zeros(
            shape=n_nodes, dtype=np.uint8)
        DOUBLE_t n_correct_predictions
        SIZE_t node_idx

    predictions_arr = np.argmax(np.asarray(orig_tree.predict(validation_set_X)), axis=1)
    validation_set_y_arr = np.asarray(validation_set_y).reshape(predictions_arr.shape[0])
    n_correct_predictions = 0
    for i in range(predictions_arr.shape[0]):
        if(predictions_arr[i] == validation_set_y_arr[i]):
            n_correct_predictions += 1
    validation_error = 1 - (n_correct_predictions / predictions_arr.shape[0])

    pruned = _reduced_error_prune(leaves_in_subtree, orig_tree, offset)
    
    if pruned:
        _build_pruned_tree(tree, orig_tree, leaves_in_subtree,
                                n_nodes)

        predictions_arr = np.argmax(np.asarray(tree.predict(validation_set_X)), axis=1)
        n_correct_predictions = 0
        for i in range(predictions_arr.shape[0]):
            if(predictions_arr[i] == validation_set_y_arr[i]):
                n_correct_predictions += 1
        pruned_error = 1 - (n_correct_predictions / predictions_arr.shape[0])

        if pruned_error <= validation_error:
            return True
